...
```JavaScript
## Prompt3（阶段三）：HLSIP → VivadoBlockDesign → PS 端 Python 验证（PYNQ-Z2）

你是一名资深 FPGA SoC 集成工程师（Vitis HLS + Vivado + PYNQ-Z2）。你的任务不是写论文，而是在**不改算法语义**的前提下，把现有 HLS 顶层核做成**可在 ZynqPS–PL 系统中部署并可由 Python 驱动验证**的工程闭环。
...
#### D) VivadoBlockDesign 集成与导出（Tcl 自动化）
...
- **HLSTcl 锁定工作目录**：`run_hls.tcl` 必须 `cd $SCRIPT_DIR`，避免产物路径随启动目录漂移。
...
#### 6.1Jupyter 运行环境差异（否则脚本在 Notebook 直接报错）
...
#### 6.2PYNQ overlay 资源路径与权限（否则 scp/运行找不到文件）
...
#### 6.3PYNQ 寄存器访问策略（必须避免 register_map 兼容性坑）
