...
int main(int argc, char** argv) {
    ...
    // Acceptance criteria
    // Allow some error due to fixed point precision and iterations
    // 0.5 pixel is a reasonable threshold for fixed point vs float
    if (max_err_u < 1.0 && max_err_v < 1.0) {
        std::cout << "Test PASSED! (Max Error < 1.0)" << std::endl;
        return 0;
    } else {
        std::cout << "Test FAILED (Max Error >= 1.0). Check precision." << std::endl;
        // Depending on strictness, we might return 1.
        // For now, let's return 0 if mean error is small, because max error can be outliers at boundaries.
        if ((total_err_u / (height*width)) < 0.1) {
             std::cout << "Test PASSED with warnings (Mean error is low)." << std::endl;
             return 0;
        }
        return 1;
    }
