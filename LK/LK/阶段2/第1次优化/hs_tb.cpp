...
// Golden Reference (Float implementation of the same algorithm)
void golden_HS(
    const cv::Mat& im1,
    const cv::Mat& im2,
    cv::Mat& vx,
    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            // Boundary handling matching HLS (Replicate)
    ...
    for (int k = 0; k < iterations; k++) {
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                ...
                // Avoid division by zero if alpha=0 (but alpha=1 here)
...
void save_flow_map(const cv::Mat& img, const cv::Mat& u, const cv::Mat& v, const std::string& filename, int step, float scale, float arrow_scale) {
    ...
    for (int y = 0; y < canvas.rows; y += step) {
        for (int x = 0; x < canvas.cols; x += step) {
            ...
            // Draw arrow
            // arrow_scale amplifies the motion vector for better visibility
            // tipLength=0.3 controls arrow head size
...
int main(int argc, char** argv) {
    (void)argc;
    (void)argv;
    std::string img1_path = "/root/autodl-tmp/whp/Vitis-Libraries-LK/vision/data/lkof_im0_256x128.png";
    std::string img2_path = "/root/autodl-tmp/whp/Vitis-Libraries-LK/vision/data/lkof_im1_256x128.png";
    ...
    if (height != MAX_HEIGHT || width != MAX_WIDTH) {
         std::cout << "Image size mismatch (expected fixed size from header)." << std::endl;
         return 1;
    }

    // Allocate memory for HLS
    // Using vector to manage memory, but passing raw pointers
    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            // Convert HLS output (Q6.10) to float
            ...
            float gold_u = golden_vx.at<float>(i, j);
            float gold_v = golden_vy.at<float>(i, j);
            
            float err_u = std::abs(hls_u_f - gold_u);
            float err_v = std::abs(hls_v_f - gold_v);
    ...
    std::cout << "Mean Error U: " << total_err_u / (height*width) << std::endl;
    std::cout << "Mean Error V: " << total_err_v / (height*width) << std::endl;
    
    // Write output images (normalized for visibility)
    cv::imwrite("hls_out_vx.png", hls_out_vx * 10 + 128);
    cv::imwrite("hls_out_vy.png", hls_out_vy * 10 + 128);
    
    // Save Quiver Plot (Arrows)
    // Step=5 and Arrow Scale=5.0 to match Matlab baseline (plotFlow.m)
    save_flow_map(img1, hls_out_vx, hls_out_vy, "hls_out_flow_arrows.png", 5, 1.0, 5.0);

    // --- Metric Calculation for Paper (AEE & AAE) ---
    // In optical flow research, Average Endpoint Error (AEE/EPE) and Average Angular Error (AAE)
    // are standard metrics to evaluate accuracy against a Golden Reference (Float Model).
    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            float u_hls = hls_out_vx.at<float>(i, j);
            float v_hls = hls_out_vy.at<float>(i, j);
            float u_gold = golden_vx.at<float>(i, j);
            float v_gold = golden_vy.at<float>(i, j);

            // 1. Endpoint Error (EPE)
            // Euclidean distance between the estimated flow vector and the ground truth vector.
            double dist_sq = std::pow(u_hls - u_gold, 2) + std::pow(v_hls - v_gold, 2);
            double epe = std::sqrt(dist_sq);
            
            total_epe += epe;
            if (epe > max_epe) max_epe = epe;

            // 2. Angular Error (AAE)
            // Angle between the estimated flow vector (u, v, 1) and ground truth (u_gt, v_gt, 1) in 3D space-time.
            // Formula: arccos( (1 + u*u_gt + v*v_gt) / (sqrt(1+u^2+v^2) * sqrt(1+u_gt^2+v_gt^2)) )
            double dot = 1.0 + u_hls * u_gold + v_hls * v_gold;
            double mag_hls = std::sqrt(1.0 + u_hls * u_hls + v_hls * v_hls);
            double mag_gold = std::sqrt(1.0 + u_gold * u_gold + v_gold * v_gold);
            
            // Clamp value for acos to [-1, 1] to avoid NaN
            double val = dot / (mag_hls * mag_gold);
            if (val > 1.0) val = 1.0;
            if (val < -1.0) val = -1.0;
            
            double aae = std::acos(val); // Result in radians
            total_aae += aae;
    ...
    double avg_epe = total_epe / valid_pixels;
    double avg_aae = total_aae / valid_pixels;
    // Convert AAE to degrees for easier reading
    double avg_aae_deg = avg_aae * (180.0 / M_PI);

    std::cout << "\n=============================================" << std::endl;
    std::cout << "  HLS Evaluation Metrics (vs Float Golden)" << std::endl;
    std::cout << "=============================================" << std::endl;
    std::cout << "  Average Endpoint Error (AEE): " << avg_epe << " pixels" << std::endl;
    std::cout << "  Max Endpoint Error (EPE):     " << max_epe << " pixels" << std::endl;
    std::cout << "  Average Angular Error (AAE):  " << avg_aae_deg << " degrees" << std::endl;
    std::cout << "=============================================\n" << std::endl;

    double avg_epe_thresh = get_env_double("HS_TB_AVG_EPE_THRESH", 1.0);
    double max_epe_thresh = get_env_double("HS_TB_MAX_EPE_THRESH", 10.0);
    double avg_aae_deg_thresh = get_env_double("HS_TB_AVG_AAE_DEG_THRESH", 10.0);

    std::cout << "TB Thresholds: AVG_EPE<=" << avg_epe_thresh
              << ", MAX_EPE<=" << max_epe_thresh
              << ", AVG_AAE_DEG<=" << avg_aae_deg_thresh
              << std::endl;

    bool pass = (avg_epe <= avg_epe_thresh) && (max_epe <= max_epe_thresh) && (avg_aae_deg <= avg_aae_deg_thresh);
    if (pass) {
        std::cout << "Test PASSED!" << std::endl;
        return 0;
    }

    std::cout << "Test FAILED (threshold exceeded)." << std::endl;
    return 1;
}
