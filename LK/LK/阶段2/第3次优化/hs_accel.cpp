...
int hls_HS(
    ...
    grad_row0: for (int j = 0; j < MAX_WIDTH; j++) {
        ...
        int idx = (j + 2 < MAX_WIDTH) ? (j + 2) : (MAX_WIDTH - 1);
    ...
    int la_l = row_above[0];
    int la_c = row_above[0];
    int la_r = row_above[1];
    int lc_l = row_curr[0];
    int lc_c = row_curr[0];
    int lc_r = row_curr[1];
    int lb_l = row_below[0];
    int lb_c = row_below[0];
    int lb_r = row_below[1];
