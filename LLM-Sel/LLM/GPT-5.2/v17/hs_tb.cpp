#include "hs_config_params.h"

#include <cmath>
#include <iostream>
#include <vector>

#include <opencv2/opencv.hpp>

static void golden_HS(const cv::Mat &im1, const cv::Mat &im2, cv::Mat &vx, cv::Mat &vy, int iterations, float alpha) {
  int height = im1.rows;
  int width = im1.cols;

  vx = cv::Mat::zeros(height, width, CV_32F);
  vy = cv::Mat::zeros(height, width, CV_32F);

  cv::Mat Ix = cv::Mat::zeros(height, width, CV_32F);
  cv::Mat Iy = cv::Mat::zeros(height, width, CV_32F);
  cv::Mat It = cv::Mat::zeros(height, width, CV_32F);

  for (int i = 0; i < height; i++) {
    for (int j = 0; j < width; j++) {
      int im1_idx = (i > 0) ? i - 1 : 0;
      int ip1_idx = (i < height - 1) ? i + 1 : height - 1;
      int jm1_idx = (j > 0) ? j - 1 : 0;
      int jp1_idx = (j < width - 1) ? j + 1 : width - 1;

      float val_m1_m1 = (float)im1.at<uint8_t>(im1_idx, jm1_idx);
      float val_m1_p1 = (float)im1.at<uint8_t>(im1_idx, jp1_idx);
      float val_0_m1 = (float)im1.at<uint8_t>(i, jm1_idx);
      float val_0_p1 = (float)im1.at<uint8_t>(i, jp1_idx);
      float val_p1_m1 = (float)im1.at<uint8_t>(ip1_idx, jm1_idx);
      float val_p1_p1 = (float)im1.at<uint8_t>(ip1_idx, jp1_idx);
      float val_m1_0 = (float)im1.at<uint8_t>(im1_idx, j);
      float val_p1_0 = (float)im1.at<uint8_t>(ip1_idx, j);

      float gx = -val_m1_m1 + val_m1_p1 - 2 * val_0_m1 + 2 * val_0_p1 - val_p1_m1 + val_p1_p1;
      float gy = -val_m1_m1 - 2 * val_m1_0 - val_m1_p1 + val_p1_m1 + 2 * val_p1_0 + val_p1_p1;

      Ix.at<float>(i, j) = gx;
      Iy.at<float>(i, j) = gy;
      It.at<float>(i, j) = (float)im2.at<uint8_t>(i, j) - (float)im1.at<uint8_t>(i, j);
    }
  }

  cv::Mat u_curr = cv::Mat::zeros(height, width, CV_32F);
  cv::Mat v_curr = cv::Mat::zeros(height, width, CV_32F);
  cv::Mat u_next = cv::Mat::zeros(height, width, CV_32F);
  cv::Mat v_next = cv::Mat::zeros(height, width, CV_32F);

  for (int k = 0; k < iterations; k++) {
    for (int i = 0; i < height; i++) {
      for (int j = 0; j < width; j++) {
        int im1_idx = (i > 0) ? i - 1 : 0;
        int ip1_idx = (i < height - 1) ? i + 1 : height - 1;
        int jm1_idx = (j > 0) ? j - 1 : 0;
        int jp1_idx = (j < width - 1) ? j + 1 : width - 1;

        float u_avg = (u_curr.at<float>(im1_idx, j) + u_curr.at<float>(ip1_idx, j) + u_curr.at<float>(i, jm1_idx) +
                       u_curr.at<float>(i, jp1_idx)) *
                      0.25f;
        float v_avg = (v_curr.at<float>(im1_idx, j) + v_curr.at<float>(ip1_idx, j) + v_curr.at<float>(i, jm1_idx) +
                       v_curr.at<float>(i, jp1_idx)) *
                      0.25f;

        float ix = Ix.at<float>(i, j);
        float iy = Iy.at<float>(i, j);
        float it = It.at<float>(i, j);

        float den = alpha * alpha + ix * ix + iy * iy;
        float num = ix * u_avg + iy * v_avg + it;

        if (den < 1e-6f) {
          den = 1e-6f;
        }

        u_next.at<float>(i, j) = u_avg - ix * (num / den);
        v_next.at<float>(i, j) = v_avg - iy * (num / den);
      }
    }
    u_curr = u_next.clone();
    v_curr = v_next.clone();
  }

  vx = u_curr;
  vy = v_curr;
}

static void save_flow_map(const cv::Mat &img,
                          const cv::Mat &u,
                          const cv::Mat &v,
                          const std::string &filename,
                          int step,
                          float arrow_scale) {
  cv::Mat canvas;
  if (img.channels() == 1) {
    cv::cvtColor(img, canvas, cv::COLOR_GRAY2BGR);
  } else {
    canvas = img.clone();
  }

  for (int y = 0; y < canvas.rows; y += step) {
    for (int x = 0; x < canvas.cols; x += step) {
      float fx = u.at<float>(y, x);
      float fy = v.at<float>(y, x);

      if (std::abs(fx) < 0.1f && std::abs(fy) < 0.1f) {
        continue;
      }

      cv::Point pt1(x, y);
      cv::Point pt2(cvRound(x + fx * arrow_scale), cvRound(y + fy * arrow_scale));
      cv::arrowedLine(canvas, pt1, pt2, cv::Scalar(0, 0, 255), 2, 8, 0, 0.3);
    }
  }
  cv::imwrite(filename, canvas);
}

int main(int argc, char **argv) {
  if (argc != 3) {
    std::cout << "Usage: " << argv[0] << " <img1> <img2>" << std::endl;
    return 1;
  }

  std::string img1_path = argv[1];
  std::string img2_path = argv[2];

  std::cout << "Loading images: " << img1_path << " and " << img2_path << std::endl;

  cv::Mat img1 = cv::imread(img1_path, cv::IMREAD_GRAYSCALE);
  cv::Mat img2 = cv::imread(img2_path, cv::IMREAD_GRAYSCALE);

  if (img1.empty() || img2.empty()) {
    std::cout << "Error loading images." << std::endl;
    return 1;
  }

  int height = img1.rows;
  int width = img1.cols;

  std::cout << "Image Size: " << width << "x" << height << std::endl;

  if (height > MAX_HEIGHT || width > MAX_WIDTH) {
    std::cout << "Image size exceeds MAX limits defined in header." << std::endl;
    return 1;
  }

  std::vector<unsigned short> hls_in1(MAX_HEIGHT * MAX_WIDTH);
  std::vector<unsigned short> hls_in2(MAX_HEIGHT * MAX_WIDTH);
  std::vector<signed short> hls_vx(MAX_HEIGHT * MAX_WIDTH);
  std::vector<signed short> hls_vy(MAX_HEIGHT * MAX_WIDTH);

  for (int i = 0; i < height; i++) {
    for (int j = 0; j < width; j++) {
      hls_in1[i * width + j] = img1.at<uint8_t>(i, j);
      hls_in2[i * width + j] = img2.at<uint8_t>(i, j);
      hls_vx[i * width + j] = 0;
      hls_vy[i * width + j] = 0;
    }
  }

  std::cout << "Calling HLS function..." << std::endl;
  hls_HS(hls_in1.data(), hls_in2.data(), hls_vx.data(), hls_vy.data(), height, width);
  std::cout << "HLS function returned." << std::endl;

  std::cout << "Running Golden Reference..." << std::endl;
  cv::Mat golden_vx, golden_vy;
  golden_HS(img1, img2, golden_vx, golden_vy, N_ITER, (float)ALPHA_VAL);

  float max_err_u = 0.0f;
  float max_err_v = 0.0f;
  float total_err_u = 0.0f;
  float total_err_v = 0.0f;

  cv::Mat hls_out_vx(height, width, CV_32F);
  cv::Mat hls_out_vy(height, width, CV_32F);

  for (int i = 0; i < height; i++) {
    for (int j = 0; j < width; j++) {
      short raw_u = hls_vx[i * width + j];
      short raw_v = hls_vy[i * width + j];

      float hls_u_f = (float)raw_u / (float)(1 << FLOW_FRAC_BITS);
      float hls_v_f = (float)raw_v / (float)(1 << FLOW_FRAC_BITS);

      hls_out_vx.at<float>(i, j) = hls_u_f;
      hls_out_vy.at<float>(i, j) = hls_v_f;

      float gold_u = golden_vx.at<float>(i, j);
      float gold_v = golden_vy.at<float>(i, j);

      float err_u = std::abs(hls_u_f - gold_u);
      float err_v = std::abs(hls_v_f - gold_v);

      if (err_u > max_err_u) {
        max_err_u = err_u;
      }
      if (err_v > max_err_v) {
        max_err_v = err_v;
      }
      total_err_u += err_u;
      total_err_v += err_v;
    }
  }

  std::cout << "Max Error U: " << max_err_u << std::endl;
  std::cout << "Max Error V: " << max_err_v << std::endl;
  std::cout << "Mean Error U: " << total_err_u / (height * width) << std::endl;
  std::cout << "Mean Error V: " << total_err_v / (height * width) << std::endl;

  cv::imwrite("hls_out_vx.png", hls_out_vx * 10.0f + 128.0f);
  cv::imwrite("hls_out_vy.png", hls_out_vy * 10.0f + 128.0f);

  save_flow_map(img1, hls_out_vx, hls_out_vy, "hls_out_flow_arrows.png", FLOW_VIS_STEP, FLOW_ARROW_SCALE);

  double total_epe = 0.0;
  double total_aae = 0.0;
  double max_epe = 0.0;
  int valid_pixels = 0;

  for (int i = 0; i < height; i++) {
    for (int j = 0; j < width; j++) {
      float u_hls = hls_out_vx.at<float>(i, j);
      float v_hls = hls_out_vy.at<float>(i, j);
      float u_gold = golden_vx.at<float>(i, j);
      float v_gold = golden_vy.at<float>(i, j);

      double dist_sq = std::pow(u_hls - u_gold, 2) + std::pow(v_hls - v_gold, 2);
      double epe = std::sqrt(dist_sq);

      total_epe += epe;
      if (epe > max_epe) {
        max_epe = epe;
      }

      double dot = 1.0 + u_hls * u_gold + v_hls * v_gold;
      double mag_hls = std::sqrt(1.0 + u_hls * u_hls + v_hls * v_hls);
      double mag_gold = std::sqrt(1.0 + u_gold * u_gold + v_gold * v_gold);

      double val = dot / (mag_hls * mag_gold);
      if (val > 1.0) val = 1.0;
      if (val < -1.0) val = -1.0;

      double aae = std::acos(val);
      total_aae += aae;
      valid_pixels++;
    }
  }

  double avg_epe = total_epe / valid_pixels;
  double avg_aae = total_aae / valid_pixels;
  double avg_aae_deg = avg_aae * (180.0 / M_PI);

  std::cout << "\n=============================================" << std::endl;
  std::cout << "  HLS Evaluation Metrics (vs Float Golden)" << std::endl;
  std::cout << "=============================================" << std::endl;
  std::cout << "  Average Endpoint Error (AEE): " << avg_epe << " pixels" << std::endl;
  std::cout << "  Max Endpoint Error (EPE):     " << max_epe << " pixels" << std::endl;
  std::cout << "  Average Angular Error (AAE):  " << avg_aae_deg << " degrees" << std::endl;
  std::cout << "=============================================\n" << std::endl;

  if (max_err_u < 1.0f && max_err_v < 1.0f) {
    std::cout << "Test PASSED! (Max Error < 1.0)" << std::endl;
    return 0;
  }

  std::cout << "Test FAILED (Max Error >= 1.0). Check precision." << std::endl;
  if ((total_err_u / (height * width)) < 0.1f) {
    std::cout << "Test PASSED with warnings (Mean error is low)." << std::endl;
    return 0;
  }
  return 1;
}
