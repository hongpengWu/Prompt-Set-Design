...
static void draw_line(uint8_t *rgb, int width, int height, int x0, int y0, int x1, int y1, uint8_t r, uint8_t g, uint8_t b, int thickness) {
    ...
    while (1) {
        if (t == 1) {
            set_pixel_rgb(rgb, width, height, x0, y0, r, g, b);
        } else if (t == 2) {
            set_pixel_rgb(rgb, width, height, x0, y0, r, g, b);
            set_pixel_rgb(rgb, width, height, x0 - 1, y0, r, g, b);
            set_pixel_rgb(rgb, width, height, x0, y0 - 1, r, g, b);
            set_pixel_rgb(rgb, width, height, x0 - 1, y0 - 1, r, g, b);
        } else {
            int radius = t / 2;
...
static void draw_arrow(uint8_t *rgb, int width, int height, int x0, int y0, int x1, int y1, uint8_t r, uint8_t g, uint8_t b) {
    ...
    draw_line(rgb, width, height, x0, y0, x1, y1, r, g, b, 2);
    ...
    draw_line(rgb, width, height, x1, y1, wing1_x, wing1_y, r, g, b, 2);
    draw_line(rgb, width, height, x1, y1, wing2_x, wing2_y, r, g, b, 2);
}
...
int main(int argc, char **argv) {
    ...
    const int flow_skip_th = (FLOW_SCALE + 5) / 10;
    ...
    for (int y = 0; y < height; y += step) {
        for (int x = 0; x < width; x += step) {
            ...
            draw_arrow(vis_rgb, width, height, x, y, x2, y2, 255, 0, 0);
        }
    }

    const char *out_path = "/home/whp/Desktop/XLab/M2HLS/hls_out_flow13.png";
