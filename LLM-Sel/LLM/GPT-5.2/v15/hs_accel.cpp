...
int hls_HS(
    ...
    for (int i = 0; i < rows; i++) {
        ...
        for (int j = 0; j < cols; j++) {
            #pragma HLS LOOP_TRIPCOUNT min=1 max=MAX_WIDTH
            #pragma HLS PIPELINE II=1
            int im1 = clamp_idx(i - 1, 0, (int)rows - 1);
            int ip1 = clamp_idx(i + 1, 0, (int)rows - 1);
            int jm1 = clamp_idx(j - 1, 0, (int)cols - 1);
            int jp1 = clamp_idx(j + 1, 0, (int)cols - 1);

            pixel_t a00 = buf_in1[im1][jm1];
            pixel_t a01 = buf_in1[im1][j];
            pixel_t a02 = buf_in1[im1][jp1];
            pixel_t a10 = buf_in1[i][jm1];
            pixel_t a11 = buf_in1[i][j];
            pixel_t a12 = buf_in1[i][jp1];
            pixel_t a20 = buf_in1[ip1][jm1];
            pixel_t a21 = buf_in1[ip1][j];
            pixel_t a22 = buf_in1[ip1][jp1];

            int gx = sobel_x_i32(a00, a01, a02, a10, a11, a12, a20, a21, a22);
            int gy = sobel_y_i32(a00, a01, a02, a10, a11, a12, a20, a21, a22);

            buf_Ix[i][j] = (grad_i16_t)(gx >> SOBEL_NORM_SHIFT);
            buf_Iy[i][j] = (grad_i16_t)(gy >> SOBEL_NORM_SHIFT);
    ...
    for (int iter = 0; iter < N_ITER; iter++) {
        ...
        for (int i = 0; i < rows; i++) {
            ...
            for (int j = 0; j < cols; j++) {
                ...
                flow_t u_bar = (buf_u[im1][j] + buf_u[ip1][j] + buf_u[i][jm1] + buf_u[i][jp1]) / 4;
                flow_t v_bar = (buf_v[im1][j] + buf_v[ip1][j] + buf_v[i][jm1] + buf_v[i][jp1]) / 4;

                acc_t ix = (acc_t)buf_Ix[i][j];
                acc_t iy = (acc_t)buf_Iy[i][j];
                acc_t it = (acc_t)buf_It[i][j];
