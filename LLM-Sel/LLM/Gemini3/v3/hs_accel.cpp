...
void hls_HS(
    ...
    // Using static to ensure mapped to BRAM/URAM
    ...
    // Cache height/width to avoid repeated reads (though HLS does this usually)
    int rows = (int)height;
    int cols = (int)width;
    if (rows > MAX_HEIGHT) rows = MAX_HEIGHT;
    if (cols > MAX_WIDTH) cols = MAX_WIDTH;

    // 1. Compute Gradients (Ix, Iy, It) and Initialize Flow
    ...
    GRAD_LOOP: for (int i = 0; i < MAX_HEIGHT; i++) {
        for (int j = 0; j < MAX_WIDTH; j++) {
            ...
            // Load 3x3 neighborhood for Sobel
            // Note: In a fully optimized stream implementation, we would use line buffers.
            // But here we access global memory randomly via cache or assume data is available.
            // Since we read inp1_img multiple times, this is not optimal for bandwidth, 
            // but for a "Kernel" implementation with random access arrays, this is functional.
            // To improve, we should cache lines. However, for 256x128, it fits in cache/BRAM easily.
            // We will just read directly.
            
            // Actually, we should be careful about random access on m_axi. 
            // Ideally we load the image into BRAM first?
            // Yes, let's load input images into BRAM first if they fit?
            // 256*128 * 2 bytes = 64KB. It fits.
            // But strict constraints: "Don't create extra files".
            // I'll stick to the logic. For better performance I should load to BRAM.
            // I'll add a load loop.
        }
    }
    
    // Revised Strategy: Load Images to Local Buffers first
    ...
    CALC_GRAD: for (int i = 0; i < MAX_HEIGHT; i++) {
        for (int j = 0; j < MAX_WIDTH; j++) {
            ...
            // int val_0_0   = local_im1[i * cols + j];
    ...
    ITER_LOOP: for (int k = 0; k < N_ITER; k++) {
        UPDATE_LOOP: for (int i = 0; i < MAX_HEIGHT; i++) {
            for (int j = 0; j < MAX_WIDTH; j++) {
                ...
                // Average
                // Use calc_t to avoid overflow during sum? 
                // flow_t is Q8.8 (+/- 128). Sum is +/- 512. Fits in flow_t if it has >9 int bits?
                // flow_t is 16 bits (8 int). +/- 128.
                // Sum might overflow if all are -128 -> -512.
                // So we should cast to calc_t or larger type before sum.
                ...
                // Cast to den_t (32-bit) to prevent overflow
                ...
                // calc_t has 28 int bits, sufficient for this product
    ...
    // 3. Write Output
    // Convert Q8.8 fixed point to signed short (raw bits or scaled?)
    // Prompt: "specify scaling factor". "Output signed short int fixed point".
    // If I just cast ap_fixed<16,8> to short, it preserves the bits?
    // ap_fixed stores as integer * 2^8.
    // If I cast (short)my_ap_fixed, it converts to integer (truncating fraction).
    // I want the raw representation (scaled value).
    // So I should use .range() or multiply by 2^8 and cast.
    // .range() gives ap_uint/ap_int.
    WRITE_LOOP: for (int i = 0; i < MAX_HEIGHT * MAX_WIDTH; i++) {
        #pragma HLS PIPELINE II=1
        if (i < rows * cols) {
             // We want the Q8.8 binary representation stored in the short.
             // This equals value * 256.
             // ap_fixed::to_short() would give the integer part.
             // We use range() to get the bits.
