...
void save_flow_map(const cv::Mat& img, const cv::Mat& u, const cv::Mat& v, const std::string& filename, int step, float scale, float arrow_scale) {
    ...
    for (int y = 0; y < canvas.rows; y += step) {
        for (int x = 0; x < canvas.cols; x += step) {
            ...
            // Skip small flow (Optional, but kept from reference TB)
            ...
            // Draw arrow
            // Thickness=2 to match Matlab LineWidth=2
...
int main(int argc, char** argv) {
    ...
    // Output path: hls_out_flow4.png (As requested)
    std::string out_flow_path = "/home/whp/Desktop/XLab/M2HLS/hls_out_flow4.png";
    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            // Convert HLS output (Q8.8) to float
    ...
    // Step=5 and Arrow Scale=5.0 to match Matlab baseline
    save_flow_map(img1, hls_out_vx, hls_out_vy, out_flow_path, 5, 1.0, 5.0);
    std::cout << "Saved flow visualization to " << out_flow_path << std::endl;

    // --- Metric Calculation (AEE & AAE) ---
